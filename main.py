...
def _load_ircore():
    ...
    try:
        ...
        for fn in (lib.hmf, lib.gauss, lib.corrct, lib.pct_standardize, lib.extrap_sib, lib.minmax):
            fn.restype = None
        return lib
...
def _make_iron():
    stops = [(0., (0, 0, 0)), (0.25, (.5, 0, .5)), (.5, (1, 0, 0)),
             (.75, (1, .6, 0)), (1., (1, 1, 1))]
...
def hmf(A):
    ...
    return np.median(np.stack([np.median(_g(Af, cr), axis=1), np.median(_g(Af, di), axis=1),
...
def skf_stats(seq):
    ...
    if _lib:
        ...
        fifth[:] = (m ** 5).mean(axis=1) / (sig2 ** 2 * sig)
