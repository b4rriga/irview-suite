...
/* --- 9-element sorting network (median filter core) --- */
...
void hmf_f32(const float *src, float *dst, int H, int W)
{
    ...
    for (int r = 0; r < H; r++) {
        for (int c = 0; c < W; c++) {
            ...
            tmp[r * W + c] =
                _median3(_median9(cr), _median9(di), src[r * W + c]);
        }
    }
             ...
             int HW, int N)
{
    for (int p = 0; p < HW; p++) {
        ...
        double m2 = 0.0, m3 = 0.0, m4 = 0.0, m5 = 0.0, rx = 0.0;
        ...
        quinto[p] = (sig5 > 1e-30) ? (float)(m5 / sig5) : 0.0f;
    }
}
